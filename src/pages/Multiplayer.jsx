...
export default function Multiplayer() {
  ...
  async function leaveLobby() {
    ...
    try {
      ...
      const me = updatedPlayers[currentUser.uid];
      ...
      if (humansLeft.length === 0) {
        // No humans left, let's just mark empty or delete. We'll simulate deletion by wiping it.
        await updateDoc(ref, { status: 'abandoned' });
        // Can't delete easily without admin, but setting status abandoned is fine for now
      } else {
        const updates = {
        ...
        if (me?.isCaptain) {
          ...
          if (teammates.length > 0) {
            const nextCap = teammates[0];
            updates[`players.${nextCap.uid}.isCaptain`] = true;
          }
        }
      ...
      <div className="container center text-center" style={{ color: "white" }}>
        <div className="card">
          ...
          <div className="form-group">
            ...
            <div style={{ margin: '20px 0', borderBottom: '1px solid rgba(255,255,255,0.2)' }}></div>
...
function LobbyRoom({ lobbyId, lobbyData, leaveLobby }) {
  ...
  async function sendMsg(e) {
    ...
    await updateDoc(doc(db, 'lobbies', lobbyId), {
      messages: [...(lobbyData.messages || []), msg]
    });
    setChatMsg('');
  }
  ...
  async function assignTeam(uid, team) {
    ...
    if (target.isCaptain) {
      updates[`players.${uid}.isCaptain`] = false; // strips captaincy when changing teams
  ...
  async function renameTeam(team, newName) {
    ...
    // For now we trust the prompt: "Captain can rename their team"
  ...
  async function startMatch() {
    ...
    const batFirst = tossWinner;
    const bowlFirst = batFirst === 'A' ? 'B' : 'A';

    await setDoc(doc(db, 'matches', lobbyId), {
       status: 'in-progress',
       matchVersion: ENGINE_CONFIG.VERSION, // 2. Version Control Hook
       innings: 1,
  ...
  const renderPlayer = (p) => (
    <div key={p.uid} style={{ 
      ...
      <div>
        ...
        {p.isReady ? <span style={{marginLeft:'8px', color:'var(--success-color)'}}>Ready</span> : <span style={{marginLeft:'8px', color:'var(--error-color)'}}>Wait</span>}
      </div>
      <div style={{display:'flex', gap:'5px'}}>
        {(isHost || p.uid === currentUser.uid) && !isAllReady && (
          <button style={{fontSize:'10px', padding:'2px 5px'}} onClick={() => assignTeam(p.uid, p.team === 'A' ? 'B' : 'A')}>Swap Team</button>
        )}
  ...
  return (
    <div className="container" style={{ color: "white", gap: "20px", flexDirection: "column", maxWidth: '1000px', margin: '0 auto'}}>
      ...
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', background:'var(--card-bg)', padding:'15px', borderRadius:'8px'}}>
        <div>
          <h2 style={{margin:0}}>Lobby Code: <span style={{color:'var(--primary-color)'}}>{lobbyId}</span></h2>
          <div style={{fontSize:'14px', color:'var(--text-secondary)'}}>
            {playersArr.length} Players | {lobbyData.overs} Overs
          </div>
        </div>
        <div style={{display:'flex', gap:'10px'}}>
          {me && (
            <button onClick={toggleReady} className="button" style={{ 
              background: me.isReady ? 'var(--success-color)' : 'gray', width: 'auto', padding: '10px 20px'
            }}>
              {me.isReady ? 'Ready' : 'Not Ready'}
            </button>
          )}
          {isHost && <button onClick={startMatch} className="button primary" style={{width:'auto'}} disabled={!isAllReady || !isValidTeams}>Start Match</button>}
      ...
      {/* Host Controls */}
      {isHost && (
        <div style={{background:'var(--card-bg)', padding:'15px', borderRadius:'8px', display:'flex', gap:'15px', alignItems:'center'}}>
          ...
          <button onClick={() => addBot('A')} className="button secondary" style={{width:'auto', padding:'5px 10px'}}>Add Bot A</button>
          <button onClick={() => addBot('B')} className="button secondary" style={{width:'auto', padding:'5px 10px'}}>Add Bot B</button>
